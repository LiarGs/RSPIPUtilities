...
class DynamicPatch : public MosaicAlgorithmBase {
  public:
    DynamicPatch(const std::vector<GeoImage> &imageDatas, const std::vector<CloudMask> &cloudMasks) : MosaicAlgorithmBase(imageDatas), _ImageDatas(imageDatas), _CloudMasks(cloudMasks) {}

    void Execute() override {
        RSPIP::Util::SortImagesByLongitude(_ImageDatas);
        RSPIP::Util::SortImagesByLongitude(_CloudMasks);
        for (int index = 0; index < _ImageDatas.size(); ++index) {
