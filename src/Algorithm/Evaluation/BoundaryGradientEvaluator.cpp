...
BoundaryGradientEvaluator::BoundaryGradientEvaluator(const Image &imageData, const Image &referenceImage, const Image &maskImage)
...
void BoundaryGradientEvaluator::Execute() {
    if (_Image.ImageData.empty() || _ReferenceImage.ImageData.empty() || _Mask.ImageData.empty()) {
        return;
    }
